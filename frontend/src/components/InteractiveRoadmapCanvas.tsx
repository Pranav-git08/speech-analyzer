...
const STAGES: StageInfo[] = [
  {
    ...
    stageNum: 'STAGE 00',
    icon: '📝',
    title: 'Candidate Registration & OTP',
    subtitle: 'High-security onboarding with email uniqueness validation and real-time password strength diagnostics.',
    tag: 'VERIFICATION',
    tagColor: '#60a5fa',
  ...
  },
  {
    ...
    stageNum: 'STAGE 01',
    icon: '🧠',
    title: '30-Min Anti-Cheat Aptitude',
    subtitle: '15 adaptive intellectual questions across Mathematical Reasoning, Time/Money/Relations, and Verbal fluency.',
    tag: 'SCREENING',
    tagColor: '#c084fc',
    gradient: 'linear-gradient(135deg, #7c3aed 0%, #a855f7 100%)',
    highlights: ['Strict >= 7/15 Passing Cutoff', 'Instant Tab-Switch Disqualification', 'Anti-Blur Proctoring'],
  ...
  },
  {
    ...
    stageNum: 'STAGE 02',
    icon: '🛡️',
    title: 'AI 5-Member GD Cohorts',
    subtitle: 'Automated clustering of qualified candidates into distinguished teams with official email venue invitations.',
    tag: 'COLLABORATION',
    tagColor: '#f472b6',
    gradient: 'linear-gradient(135deg, #db2777 0%, #ec4899 100%)',
    highlights: ['Automated 5-Candidate Clusters', 'Venue, Date & Room Scheduling', 'Motivational Rejection Letters'],
    metrics: '5 Members Per Cohort',
    ctaText: 'Explore GD Workflow ➔',
    ctaPath: '/register',
  },
  {
    id: 3,
    stageNum: 'STAGE 03',
    icon: '🔑',
    title: 'Universal VOXIS-INT Token',
    subtitle: 'Single verified access token unlocks freedom to choose between Technical (TJI) and Non-Technical (NTJI) tracks.',
    tag: 'TRACK CHOICE',
    tagColor: '#fbbf24',
  ...
  },
  {
    ...
    stageNum: 'STAGE 04',
    icon: '🎙️',
    title: 'TJI & NTJI Acoustic AI Interview',
    subtitle: 'Live English-only technical and non-technical speech interviews with real-time pitch, cadence, and eye proctoring.',
    tag: 'EVALUATION',
    tagColor: '#4ade80',
    gradient: 'linear-gradient(135deg, #059669 0%, #10b981 100%)',
    highlights: ['Real-Time Acoustic Telemetry', 'Cadence & Jitter Stability', 'AI SWOT & Radar Scorecards'],
...
export const InteractiveRoadmapCanvas: React.FC = () => {
  ...
  // Auto cycle stages every 4 seconds unless paused
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const particles: { x: number; t: number; speed: number; size: number; hue: number }[] = Array.from(
      ...
      () => ({
        ...
        hue: [210, 260, 290, 45, 150][Math.floor(Math.random() * 5)],
      })
    );
    ...
    const render = () => {
      ...
      const paddingX = Math.min(width * 0.08, 60);
      ...
      trackGrad.addColorStop(0, 'rgba(59, 130, 246, 0.2)');
      trackGrad.addColorStop(0.25, 'rgba(168, 85, 247, 0.3)');
      trackGrad.addColorStop(0.5, 'rgba(236, 72, 153, 0.3)');
      trackGrad.addColorStop(0.75, 'rgba(245, 158, 11, 0.3)');
      trackGrad.addColorStop(1, 'rgba(16, 185, 129, 0.3)');
      ...
      // 4. Render 5 Holographic Stage Nodes
      STAGES.forEach((s, idx) => {
        ...
        if (isActive) {
          ...
          auraGrad.addColorStop(0, 'rgba(96, 165, 250, 0.35)');
        ...
        // Node circle body
        ctx.beginPath();
        ctx.arc(nx, nodeY, isActive ? 16 : 11, 0, Math.PI * 2);
        if (isActive) {
        ...
        ctx.font = `${isActive ? '800 11px' : '600 10px'} sans-serif`;
        ctx.fillStyle = isActive ? '#ffffff' : '#94a3b8';
        ctx.fillText(s.title.split(' ')[0], nx, nodeY + 30);
      });
      ...
      >
        ...
        <div style={styles.cardActionRow}>
          <div style={styles.progressIndicator}>
            <span>Progress: {((activeStage + 1) / STAGES.length) * 100}% Complete</span>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  progressBarFill: {
    height: '100%',
    background: 'linear-gradient(90deg, #3b82f6 0%, #a855f7 50%, #4ade80 100%)',
    borderRadius: '999px',
